...
# -----------------------------------------------------------------------------
# [보안] 비밀번호 & API 키 설정 (100% 안전한 방식)
# -----------------------------------------------------------------------------
try:
    # Secrets에서 가져오기 (코드에 노출 X)
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        st.text_input(
            "🔑 팀 접속 비밀번호를 입력하세요", type="password", on_change=password_entered, key="password"
        )
        return False
...
with st.sidebar:
    ...
    st.success("✅ Master Logic 활성화 (로그인됨)")
...
st.markdown(
    """
    **경쟁사 대비 우리 제품을 찾아야만 하는 결정적 이유(CEP)를 도출하고, 5대 매체 레퍼런스와 실제 고객 반응(VOC)을 즉시 확인하세요.**
    """
)
...
def generate_strategy(api_key, name, target, details, platform, tone):
    ...
    **[⚠️ 심의/반려 주의 (Compliance Check)]**
    ...
    아래 정보를 바탕으로 **경쟁사 대비 우리 제품을 선택해야만 하는 이유가 담긴 CEP 7가지**를 도출하세요.
    ...
    1. **Data Gathering**: 타겟의 페인 포인트와 VOC를 분석하십시오.
    2. **Core Category**: "기능"이 아닌 소비자의 "언어"로 카테고리를 재정의하십시오.
    3. **7W Expansion**: 상황을 아주 구체적으로 그리십시오.
    4. **3C Validation**: 빈도, 적합성, 경쟁을 따져 가장 유효한 7개를 선정하십시오.
    5. **Winning Point Extraction**: 경쟁사 제품이 해결해주지 못하는 우리 제품만의 차별점을 반드시 포함하십시오.
    ...
    **[중요: 검색 키워드 추출]**
    ...
    [
      {{
        ...
        "situation_summary": "7W 분석을 토대로 작성된 구체적인 상황 묘사 (1~2문장)",
...
if generate_btn:
    if not product_name or not target_audience or not product_details:
        ...
        with col2:
            with st.spinner(f"경쟁사를 압도할 '{platform}' 맞춤 전략을 수립 중입니다..."):
                # 변경됨: MY_API_KEY 사용
